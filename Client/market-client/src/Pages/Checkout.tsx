...
function getStepContent(
  ...
  productsAmount: Map<Product, number>
) {
  switch (step) {
    case 0:
...
export default function Checkout() {
  ...
  const [productsAmount, setProductsAmount] = React.useState<Map<
    Product,
    number
  > | null>(null);
    ...
    fetchResponse(serverGetCart(buyerId))
      .then((cart: Cart) => {
        ...
        fetchProducts(
          ...
          const cartProducts: CartProduct[] = convertToCartProduct(
            products,
            prodsToQuantity
          );

          const productToAmount: Map<Product, number> = cartProducts.reduce(
            (map: Map<Product, number>, cartProduct: CartProduct) => {
    ...
    return (
      <React.Fragment>
        ...
        <Typography variant='subtitle1'>
          {purchase?.purchaseDescription.split("\n").map((line) => (
            <p>
              {line}
              <br></br>
            </p>
